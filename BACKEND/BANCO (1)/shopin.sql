...
--
-- Host: 127.0.0.1
-- Tempo de geração: 13/06/2026 às 19:55
...
INSERT INTO `produto` (`cod_produto`, `cod_usuario`, `cod_categoria`, `nome`, `marca`, `descricao`, `valor`, `promocao`, `estoque`) VALUES
(5, 6, 20, 'guitarra peixe', 'fell good inc ', 'guitarra em formato de peixe', 1200.00, 1079.99, 0),
(6, 6, 20, 'Red Moon', 'fell good inc', 'A única guitarra capaz de derrubar regimes, amizades e a afinação da banda ao mesmo tempo.\r\n\r\nCansado de guitarras comuns que apenas fazem música? Apresentamos a Red Moon, uma obra-prima inspirada em uma certa ferramenta agrícola historicamente associada a revoluções e conversas extremamente desconfortáveis em aulas de história.', 1991.00, 0.00, 67);
...
(100, 'Luiz felipe', 'luiz@gmail.com', '', '(88) 99735-3939', '08895288378', '', NULL, '', NULL, NULL);
...
--
ALTER TABLE `logistica`
  ADD CONSTRAINT `fk_logistica_pedido` FOREIGN KEY (`cod_pedido`) REFERENCES `pedido` (`cod_pedido`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_logistica_usuario` FOREIGN KEY (`cod_usuario`) REFERENCES `usuario` (`cod_usuario`) ON DELETE CASCADE ON UPDATE CASCADE;
...
--
ALTER TABLE `possui`
  ADD CONSTRAINT `fk_possui_pedido` FOREIGN KEY (`cod_pedido`) REFERENCES `pedido` (`cod_pedido`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_possui_produto` FOREIGN KEY (`cod_produto`) REFERENCES `produto` (`cod_produto`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE `produto`
  ...
  ADD CONSTRAINT `fk_produto_usuario` FOREIGN KEY (`cod_usuario`) REFERENCES `usuario` (`cod_usuario`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;
