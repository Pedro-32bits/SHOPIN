...
class ProdutoDAO
{
    ...
    function inserir($produto)
    {
        ...
        $sql = "INSERT INTO produto (cod_usuario, cod_categoria, nome, marca, descricao, valor, promocao, estoque, vendidos) VALUES (:cod_usuario, :cod_categoria, :nome, :marca, :descricao, :valor, :promocao, :estoque, :vendidos)";
        ...
        $consulta->bindValue(":vendidos", $produto->getVendidos());
    ...
    function VendidoMais1($produto)
    {
        include __DIR__ . "/../conexao.php";
        $sql = "UPDATE produto SET vendidos=:vendidos WHERE cod_produto=:cod_produto";
        $consulta = $conexao->prepare($sql);
        $consulta->bindValue(":cod_produto", $produto->getCod_produto());
        $consulta->bindValue(":vendidos", $produto->getVendidos());
    ...
    function incrementarVendidos($cod_produto, $quantidade = 1)
    {
        include __DIR__ . "/../conexao.php";
        $sql = "UPDATE produto SET vendidos = COALESCE(vendidos, 0) + :quantidade WHERE cod_produto = :cod_produto";
        $consulta = $conexao->prepare($sql);
        $consulta->bindValue(":cod_produto", $cod_produto);
